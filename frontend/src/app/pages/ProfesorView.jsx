    ...
    <div className="bg-gray-950 py-10">
      <div className="mx-auto max-w-7xl space-y-10 px-4 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          <div className="rounded-[28px] border border-slate-800 bg-slate-900/95 p-8 shadow-lg">
            <h2 className="text-2xl font-semibold text-white">Registrar seguimiento</h2>
          ...
          <div className="rounded-[28px] border border-slate-800 bg-slate-900/95 p-8 shadow-lg">
            <h2 className="text-2xl font-semibold text-white">Historial de la empresa</h2>
            <form onSubmit={loadHistorial} className="mt-6 flex gap-2">
              <input
                ...
                required
              />
              <button type="submit" className="rounded-3xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-700">
                Buscar
              </button>
            </form>
            <div className="mt-4 space-y-3 max-h-48 overflow-y-auto pr-2">
              {historial.length === 0 ? (
                <p className="text-sm text-slate-500 italic text-center py-4">No se han cargado registros históricos.</p>
              ) : (
                historial.map((item, index) => (
                  <div key={index} className="rounded-2xl border border-slate-800 bg-slate-950 p-3 text-sm text-slate-300">
                    <p className="font-semibold text-slate-100">Profesor ID: {item.profesor_id}</p>
                    <p className="mt-1 text-xs text-slate-400">{item.fecha || "Fecha no registrada"}</p>
                    <p className="mt-2 bg-slate-900/50 p-2 rounded-xl text-slate-200">{item.observaciones}</p>
                  </div>
                ))
              )}
            </div>
        ...
        <section className='grid gap-6 xl:grid-cols-2'>
          <div className='rounded-[28px] border border-slate-800 bg-slate-900/95 p-8 shadow-lg'>
            <h2 className='text-2xl font-semibold text-white'>Estado de plazas</h2>
