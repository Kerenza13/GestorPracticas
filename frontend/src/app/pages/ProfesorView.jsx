...
import { importAlumnos, importEmpresas, asignarAlumno, registrarSeguimiento, verEstadoPlazas, obtenerHistorialEmpresa } from '../../api/api'
...
export function ProfesorView() {
  ...
  const handleAssignAlumno = async (event) => {
    ...
    try {
      const result = await asignarAlumno({ alumno_id: Number(assignAlumnoId), empresa_id: Number(assignEmpresaId), ciclo_id: Number(assignCicloId) })
      setMessage(result.mensaje)
      setAssignAlumnoId('')
      setAssignEmpresaId('')
      setAssignCicloId('')
  ...
  const handleSeguimiento = async (event) => {
    ...
    try {
      const result = await registrarSeguimiento({ empresa_id: Number(seguimientoEmpresaId), profesor_id: Number(profesorId), observaciones })
      setMessage(result.mensaje)
